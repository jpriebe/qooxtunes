...
class songdb:
    ...
    def lookup_m3u_tracks (self, songs):
        for s in songs:
            ...
            cursor.execute ('SELECT idSong, strTitle, strAlbumArtists, strArtists, strAlbum, iTrack FROM songview WHERE strPath=? AND strFileName=? COLLATE NOCASE', (path, basename))
            ...
            if len (rows) == 0:
                cursor.execute ('SELECT idSong, strTitle, strAlbumArtists, strArtists, strAlbum, iTrack FROM songview WHERE strPath LIKE ? AND strFileName LIKE ?', (path, basename))
                rows = cursor.fetchall ()
                for row in rows:
                    s.id = row[0]
                    s.title = row[1]
